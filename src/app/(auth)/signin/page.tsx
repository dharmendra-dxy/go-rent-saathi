...
export default function LoginForm({
  ...
  async function onSubmit(data: signinFormValues) {
    setIsLoading(true);
    const res: ActionResponse<unknown> = await signIn(
      data.email,
      data.password,
    );
    ...
    if (res?.success) {
      ...
      // fetch the user's organization to build the slug-based route
      const orgs = await authClient.organization.list();
      const slug = orgs.data?.[0]?.slug;

      router.push(slug ? UI_ROUTES.org(slug).DASHBOARD : UI_ROUTES.BASE);
    } else {
